...
{
	public class RT_WorldGenStep_Rivers : WorldGenStep
	{
		...
		private void GenerateRivers()
		{
			...
			List<int>[] array = Find.WorldPathFinder.FloodPathsWithCostForTree(coastalWaterTiles, delegate(int st, int ed)
			{
				...
				{
					if (RT_WorldGenStep_Rivers.GetImpliedElevation(Find.WorldGrid[neighbors[j]]) < RT_WorldGenStep_Rivers.GetImpliedElevation(Find.WorldGrid[num]))
					{
						num = neighbors[j];
					}
				}
				...
				return Mathf.RoundToInt(num2 * RT_WorldGenStep_Rivers.ElevationChangeCost.Evaluate(RT_WorldGenStep_Rivers.GetImpliedElevation(tile2) - RT_WorldGenStep_Rivers.GetImpliedElevation(tile)));
		...
		private void AccumulateFlow(float[] flow, List<int>[] riverPaths, int index)
		{
			...
			flow[index] = Mathf.Max(0f, flow[index] - RT_WorldGenStep_Rivers.CalculateTotalEvaporation(flow[index], tile.temperature));
		}
		...
		{
			return RT_WorldGenStep_Rivers.CalculateRiverSurfaceArea(flow);
		}

		public static float CalculateTotalEvaporation(float flow, float temperature)
		{
			return RT_WorldGenStep_Rivers.CalculateEvaporationConstant(temperature) * RT_WorldGenStep_Rivers.CalculateEvaporativeArea(flow) * 250f;
		}
	}
}
