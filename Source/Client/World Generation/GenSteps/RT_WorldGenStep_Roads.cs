...
{
	public class RT_WorldGenStep_Roads : WorldGenStep
	{
		private struct Link
		{
		...
		{
			public RT_WorldGenStep_Roads.Connectedness parent;

			public RT_WorldGenStep_Roads.Connectedness Group()
			{
				if (this.parent == null)
				{
					return this;
		...
		private void GenerateRoadEndpoints()
		{
			...
			int num = GenMath.RoundRandom((float)Find.WorldGrid.TilesCount / 100000f * RT_WorldGenStep_Roads.ExtraRoadNodesPer100kTiles.RandomInRange);
			for (int i = 0; i < num; i++)
			{
			...
			{
				int num2 = Mathf.Max(0, RT_WorldGenStep_Roads.RoadDistanceFromSettlement.RandomInRange);
				int num3 = list[j];
		...
		private void GenerateRoadNetwork()
		{
			...
			List<RT_WorldGenStep_Roads.Link> linkProspective = this.GenerateProspectiveLinks(Find.World.genData.roadNodes);
			List<RT_WorldGenStep_Roads.Link> linkFinal = this.GenerateFinalLinks(linkProspective, Find.World.genData.roadNodes.Count);
			this.DrawLinksOnWorld(linkFinal, Find.World.genData.roadNodes);
		}

		private List<RT_WorldGenStep_Roads.Link> GenerateProspectiveLinks(List<int> indexToTile)
		{
			...
			List<RT_WorldGenStep_Roads.Link> linkProspective = new List<RT_WorldGenStep_Roads.Link>();
			...
			for (srcIndex = 0; srcIndex < indexToTile.Count; srcIndex++)
			{
				...
				Find.WorldPathFinder.FloodPathsWithCost(list, (int src, int dst) => Caravan_PathFollower.CostToMove(3300, src, dst, null, true, null, null), null, delegate(int tile, float distance)
				{
					...
					{
						found++;
						linkProspective.Add(new RT_WorldGenStep_Roads.Link
						{
							distance = distance,
							indexA = srcIndex,
			...
			linkProspective.Sort((RT_WorldGenStep_Roads.Link lhs, RT_WorldGenStep_Roads.Link rhs) => lhs.distance.CompareTo(rhs.distance));
			return linkProspective;
		}

		private List<RT_WorldGenStep_Roads.Link> GenerateFinalLinks(List<RT_WorldGenStep_Roads.Link> linkProspective, int endpointCount)
		{
			List<RT_WorldGenStep_Roads.Connectedness> list = new List<RT_WorldGenStep_Roads.Connectedness>();
			for (int i = 0; i < endpointCount; i++)
			{
				list.Add(new RT_WorldGenStep_Roads.Connectedness());
			}
			List<RT_WorldGenStep_Roads.Link> list2 = new List<RT_WorldGenStep_Roads.Link>();
			int j = 0;
			while (j < linkProspective.Count)
			{
				RT_WorldGenStep_Roads.Link prospective = linkProspective[j];
				...
				{
					if (!list2.Any((RT_WorldGenStep_Roads.Link link) => link.indexB == prospective.indexA && link.indexA == prospective.indexB))
					{
						goto IL_A9;
					}
				}
				...
				if (list[prospective.indexA].Group() != list[prospective.indexB].Group())
				{
					RT_WorldGenStep_Roads.Connectedness parent = new RT_WorldGenStep_Roads.Connectedness();
		...
		private void DrawLinksOnWorld(List<RT_WorldGenStep_Roads.Link> linkFinal, List<int> indexToTile)
		{
			foreach (RT_WorldGenStep_Roads.Link current in linkFinal)
			{
